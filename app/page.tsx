...
const GenericLinkIcon = ({ className = "w-4 h-4" }: { className?: string }) => (
...
function ConstellationPattern() {
  return (
    <div className="constellation-pattern" aria-hidden>
...
export default function Home() {
  ...
  // Guard: clamp index if projects shrink
  useEffect(() => {
    if (activeProjectIdx >= projects.length && projects.length > 0) {
  ...
  return (
    <div className="max-w-3xl mx-auto px-0 sm:px-6 py-8 sm:py-10">
      ...
      <div className="x-feed">
            ...
            <section className="x-row py-10 px-5 sm:py-16 sm:px-7 scan-line">
              <div className="flex items-start gap-5 sm:gap-6 mb-6">
                <div className="relative shrink-0">
                  <div className="n-card p-0.75" style={{ width: "fit-content" }}>
                    <Image
                      ...
                      width={92}
                      height={92}
                      className="block w-21 h-21 sm:w-23 sm:h-23 object-cover"
                ...
                <div className="pt-0.5 min-w-0 flex-1">
                  ...
                  <h1 className="n-heading text-3xl sm:text-4xl mb-2 wrap-break-word">
                    {personalInfo.name}
                  </h1>
              ...
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                {personalInfo.resumeUrl === "#" && (
                  <a href={personalInfo.resumeUrl} className="n-btn opacity-60">
                    <FileText className="w-3.5 h-3.5" />
                    RESUME
                  </a>
                )}
              ...
              {hasSocials && <div className="x-line my-5" />}

              {hasSocials && (
                <div className="flex items-center gap-2">
                  <span className="coord-label mr-1 hidden sm:inline">SOCIAL</span>
                  {socials.map((s) => {
                    const Icon = socialIcons[s.icon] ?? GenericLinkIcon;
                    return (
                      <a
                        key={s.name}
                        href={s.url}
        ...
        {/* ===== PROJECTS ===== */}
        {settings.showProjects && hasProjects && active && (
          <FadeIn index={2}>
            <section className="x-row py-10 px-5 sm:py-16 sm:px-7">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-0 items-stretch sm:items-center relative z-50">
                <div className="relative flex-1" ref={dropdownRef}>
                  <button
                    ...
                    className="w-full flex items-center justify-between n-card-inner px-4 py-3 text-left focus:outline-none focus-visible:border-(--fg-muted)"
              ...
              <div
                key={projectKey}
                className="min-h-35 flex flex-col justify-between relative z-10 project-content-enter"
              >
        ...
        {/* ===== TECH STACK ===== */}
        {settings.showTechStack && hasTechStack && (
          <FadeIn index={3}>
            <section className="x-row py-10 px-5 sm:py-16 sm:px-7">
              <SectionHeader
                code="SEC_03 / TOOLS"
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-2">
                      {category.items.map((item) => (
                        ...
                        >
                          {mounted && (
                            <i
                              className={`${item.icon} text-[13px] transition-colors group-hover:text-(--fg)`}
                              style={{ color: "var(--fg-subtle)" }}
                              aria-hidden
                            />
                          )}
                          <span className="n-mono text-[11px] group-hover:text-(--fg) transition-colors">
                            {item.name}
                          </span>
                        </div>
                      ))}
              ...
              <div>
                {socials.map((s) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <Icon className="w-4 h-4 transition-colors group-hover:text-(--fg)" />
                ...
                {personalInfo.email && (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <Mail className="w-4 h-4 transition-colors group-hover:text-(--fg)" />
                      <span className="n-mono">MAIL</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="coord-label hidden sm:inline opacity-0 group-hover:opacity-100 transition-opacity truncate max-w-40">
                        {personalInfo.email}
                      </span>
                      <ArrowUpRight className="w-4 h-4 opacity-40 group-hover:opacity-100 transition-all" />
                    </div>
